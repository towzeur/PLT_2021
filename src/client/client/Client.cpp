...
public:
    ...
    const int double_quads_map[8] = {
        HEXAGON_RIGHT, HEXAGON_RIGHT_UP, HEXAGON_LEFT_UP, HEXAGON_LEFT,    // top    quad
        HEXAGON_LEFT, HEXAGON_LEFT_DOWN, HEXAGON_RIGHT_DOWN, HEXAGON_RIGHT // bottom quad
    };
...
public:
    ...
    const sf::Color COLOR_MAP[3] = {
        sf::Color(187, 153, 221),
    ...
    void initialize(){
        initialize(n_row, n_col, hexagon_r);
    }
    ...
    void initialize(unsigned int nr, unsigned int nc, float hr){
        ...
        for (int c=0; c<n_col; c++) hexa_centers_x[c] = c * (hexagon_w * 3.0 / 4.0) + hexagon_w2;

        hexa_centers_y = new int[n_row];
        for (int r=0; r<n_row; r++) hexa_centers_y[r] = (hexagon_h * r) + hexagon_h2;
    ...
    void fill_vertex(int r, int c, sf::Color col){
        ...
        m_vertices[idx+1].position = sf::Vector2f(xc + 0.5*hexagon_w2, yc  + hexagon_h2);
        m_vertices[idx+2].position = sf::Vector2f(xc - 0.5*hexagon_w2, yc  + hexagon_h2);
        m_vertices[idx+3].position = sf::Vector2f(xc - hexagon_w2, yc);
        // second quad (top)
        m_vertices[idx+4].position = m_vertices[idx+3].position;
        m_vertices[idx+5].position = sf::Vector2f(xc - 0.5*hexagon_w2, yc - hexagon_h2);
        m_vertices[idx+6].position = sf::Vector2f(xc + 0.5*hexagon_w2, yc - hexagon_h2);
    ...
    sf::Vector2u PointToCoord(double x, double y){
        ...
        int cube_x=r, cube_y=-r-q, cube_z=q;
        int col = cube_x;
...
int run(){
    ...
    sf::RenderWindow window({WIDTH, HEIGHT}, "Hexagons", sf::Style::Default, settings);
    ...
    sf::Font font;
    if (!font.loadFromFile("../res/fonts/Square.ttf")) return -1;
    ...
    while (window.isOpen()) {
            ...
            switch (event.type){
                ...
                case sf::Event::MouseButtonPressed:
                    ...
                    if (event.mouseButton.button == sf::Mouse::Right){
                        std::cout << "RIGHT (" << event.mouseButton.x << ", " << event.mouseButton.y << ")" << std::endl;
                    }
                    else if (event.mouseButton.button == sf::Mouse::Left){
                        std::cout << "LEFT (" << event.mouseButton.x << ", " << event.mouseButton.y << ")" << std::endl;
                        sf::Vector2u coords = hm.PointToCoord(event.mouseButton.x, event.mouseButton.y);
                        std::cout << "=>" << coords.x << "," << coords.y << std::endl;
                        int r_click=coords.x, c_click=coords.y;
                        if ((r_click>=0 && r_click<hm.n_row) && (c_click>=0 && c_click<hm.n_col)){
                            std::cout << "!"<< std::endl;
        ...
        */
        ...
        //fps = 1.0f / (time_curr.asSeconds() - time_prev.asSeconds()); // the asSeconds returns a float
        //time_prev = time_curr;
        ...
        if (clock.getElapsedTime().asSeconds() > 1.f){
            ...
            //std::cout << "[FPS] " << floor(frame) << std::endl; // flooring it will make the frame rate a rounded number
