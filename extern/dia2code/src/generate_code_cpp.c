...
static char *nospc(char *str) { return subst(str, ' ', '_'); }
...
static void add_setter_getter(umlclassnode *node, char *typename,
                              char *varname) {
  int foundGetter = 0;
  int foundSetter = 0;
  ...
  if (strlen(name) > 0) {
    ...
    if (!si->sfmlGraphics &&
        (strstr(name, "sf::RenderWindow") || strstr(name, "sf::VertexArray") ||
         ...
         strstr(name, "sf::RenderStates"))) {
      print("#include <SFML/Graphics.hpp>\n");
      si->sfmlGraphics = 1;
    }
    if (!si->sfmlAudio &&
        (strstr(name, "sf::Sound") || strstr(name, "sf::SoundBuffer") ||
    ...
    if (!si->tgui && ((strstr(name, "tgui::") == name) ||
                      (strstr(name, "std::shared_ptr<tgui::") == name))) {
